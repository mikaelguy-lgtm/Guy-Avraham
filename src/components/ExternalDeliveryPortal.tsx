...
const downloadBlob = (blob: Blob, filename: string, open = false) => {
  if (open) {openFreshPdfBlob(blob, filename); return;}
...
function MaskedCase({snapshot}: {snapshot: Record<string, unknown>}) {
  const data = snapshot as {
    ...
    creditIndication?: ExternalCreditIndication | null;
    dealDetails?: string; documentStatus?: string;
  };
  return <div className="external-case-grid"><article className="external-summary-card"><h2>תקציר העסקה</h2><dl><div><dt>מטרת ההלוואה</dt><dd>{formatLoanPurpose(data.loanRequest?.purpose ?? "")}</dd></div>{data.loanRequest?.purpose === "OTHER" && data.loanRequest.purposeOther && <div><dt>פירוט מטרת ההלוואה</dt><dd>{data.loanRequest.purposeOther}</dd></div>}<div><dt>סכום מבוקש</dt><dd>{formatCurrency(data.loanRequest?.requestedAmount ?? 0)}</dd></div><div><dt>שווי הנכס</dt><dd>{formatCurrency(data.property?.value ?? 0)}</dd></div><div><dt>אחוז מימון</dt><dd>{data.loanRequest?.loanToValue ?? 0}%</dd></div><div><dt>תקופה</dt><dd>{data.loanRequest?.requestedTermMonths ?? 0} חודשים</dd></div><div><dt>סוג נכס</dt><dd>{formatPropertyType(data.property?.propertyType ?? "")}</dd></div><div><dt>עיר הנכס</dt><dd>{data.property?.city || "לא צוין"}</dd></div></dl></article>
    ...
    {data.creditIndication && <article className="external-wide external-summary-card"><h2>חיווי אשראי</h2><p className="external-section-subtitle">האם היו ב-3 השנים האחרונות:</p><dl>{creditIndicationRows(data.creditIndication).map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></article>}
