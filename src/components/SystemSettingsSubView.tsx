...
export default function SystemSettingsSubView({user}: {user: CurrentUser}) {
  return <main className="admin-page"><section className="panel"><h1>הגדרות מערכת</h1><p>הגדרות תפעול, אבטחה ושירותים חיצוניים.</p>
    {user.role === "SUPER_ADMIN" ? <>
      ...
      <Link className="settings-link" to="/admin/settings/legal"><strong>מסמכים משפטיים</strong><span>תנאי שימוש ומדיניות פרטיות — עריכה, תצוגה מקדימה ופרסום גרסאות</span></Link>
    </> : <p className="permission-note">הגדרות דואר יוצא ומסמכים משפטיים זמינות לסופר אדמין בלבד.</p>}
  </section></main>;
}
