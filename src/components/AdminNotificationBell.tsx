...
export default function AdminNotificationBell() {
  ...
  const open_item = (id: number, link: string) => { void markRead(id); setOpen(false); navigate(link); };
  ...
  return <div className="notification-bell" ref={container}>
    ...
    {open && <section className="notification-popover content-card" role="dialog" aria-label="התראות אחרונות">
      ...
      <div className="notification-popover-list">{loading ? <p className="notification-popover-empty">טוען התראות…</p> : notifications.length === 0 ? <p className="notification-popover-empty">אין התראות חדשות.</p> : notifications.slice(0, 8).map((notification) => <button type="button" className={`notification-popover-item${notification.readAt ? " read" : ""}`} key={notification.id} onClick={() => open_item(notification.id, adminEntityLink(notification.entityType, notification.entityId))}><span className="notification-dot" /><span><strong>{notification.title}</strong><small>{notification.body}</small><time>{formatDate(notification.createdAt)}</time></span></button>)}</div>
      <Link className="notification-all-link" to="/admin/notifications" onClick={() => setOpen(false)}>לכל ההתראות</Link>
    </section>}
  </div>;
}
