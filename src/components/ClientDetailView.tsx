...
import { Building2, Edit3, FileUp, Send, X } from "lucide-react";
import { useLocation, useNavigate, useParams, useSearchParams } from "react-router-dom";
import type { Client, ClientBorrower, ClientLiability, ClientSubmission, LoanOffer } from "../types";
...
import { formatAdditionalIncomeType, formatBorrowerRelationship, formatClientStatus, formatCurrency, formatDate, formatEmploymentType, formatLiabilityType, formatLoanPurpose, formatMaritalStatus, formatOfferStatus, formatPropertyType, formatSubmissionStatus, maskIdentityNumber } from "../utils/formatters";
...
export default function ClientDetailView() {
  ...
  const load = useCallback(async () => {
    const [clientResult, submissionResult, offerResult] = await Promise.all([api.client(clientId), api.submissions(clientId), api.offers(clientId)]);
  ...
  return <main className="advisor-page client-detail-page">{toast && <div className="toast success" role="status"><strong>{toast}</strong><button type="button" aria-label="סגירת הודעה" onClick={() => setToast(null)}><X size={16} /></button></div>}
    ...
    <section className="content-card tab-content">
      {tab === "summary" && <div className="summary-grid"><article><h2>תמונת מצב</h2><dl>{info("סטטוס התיק", formatClientStatus(client.status))}{info("מספר לווים", client.numberOfBorrowers)}{info("הקשר בין הלווים", formatBorrowerRelationship(client.borrowerRelationship))}{info("מטרת ההלוואה", formatLoanPurpose(client.loanPurpose))}{info("סטטוס מול חברות", formatSubmissionStatus(latestSubmission?.status ?? null))}{info("מספר הצעות", offers.length)}{info("עודכן לאחרונה", formatDate(client.updatedAt))}</dl></article><article><h2>נתונים פיננסיים</h2><dl>{info("סך הכנסה חודשית", formatCurrency(client.totalMonthlyIncome))}{info("סך יתרות התחייבויות", formatCurrency(client.totalLiabilityBalance))}{info("סך החזרים חודשיים", formatCurrency(client.totalMonthlyPayments))}{info("מספר התחייבויות", client.activeLiabilityCount)}</dl>{client.missingRequiredDocumentCount > 0 && <button type="button" className="document-alert" onClick={() => selectTab("documents")}>בתיק חסרים מסמכי חובה <b>{client.missingRequiredDocumentCount}</b></button>}</article></div>}
      ...
      {tab === "deal-details" && <div className="detail-section"><div className="section-heading compact"><div><h2>פירוט עסקה</h2><p>מידע זה אינו נכלל בתיק האנונימי ללא אישור מפורש.</p></div>{editButton("deal-details", "עריכת פירוט עסקה")}</div><dl className="detail-grid">{info("מטרת ההלוואה", formatLoanPurpose(client.loanPurpose))}{info("עודכן לאחרונה", formatDate(client.dealDetailsUpdatedAt))}{info("עודכן על ידי", client.dealDetailsUpdatedBy)}</dl><div className="deal-details-text">{client.dealDetails}</div></div>}
      {tab === "documents" && <DocumentManager clientId={client.id} borrowers={client.borrowers} onUpdated={() => void load()} />}
      {tab === "offers" && <div className="detail-section"><h2>הצעות מימון</h2>{offers.length === 0 ? <div className="empty-state">עדיין לא התקבלו הצעות לתיק זה.</div> : <div className="offers-grid">{offers.map((offer) => <article className="offer-card" key={offer.id}><span className="stat-icon green"><Building2 /></span><h3>{offer.lenderName}</h3><span>{formatOfferStatus(offer.status)}</span><strong>{formatCurrency(offer.amount)}</strong></article>)}</div>}</div>}
    </section>
