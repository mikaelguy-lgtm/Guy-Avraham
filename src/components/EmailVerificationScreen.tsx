...
export default function EmailVerificationScreen({onAuthenticated}: {onAuthenticated: (user: CurrentUser) => void}) {
  ...
  const [message, setMessage] = useState(locationState?.verificationEmailSent ? emailServerAcceptedMessage(locationState.email) : "");
  const [sendAttempts, setSendAttempts] = useState(locationState?.verificationEmailSent ? 1 : 0);
  const [error, setError] = useState("");
  const [requestId, setRequestId] = useState("");
  const navigate = useNavigate();
  const cooldownSeconds = useResendCooldown(status.lastSentAt);
  ...
  const resend = async () => {
    ...
    try {
      ...
      setSendAttempts((current) => current + 1);
      setMessage(emailServerAcceptedMessage(status.email));
    } catch (caught) {
      const apiError = caught instanceof ApiError ? caught : null;
  ...
  const statusLabel = status.status === "SENT" ? "נשלח לשרת הדואר" : status.status === "FAILED" ? "השליחה נכשלה" : "טרם נשלח";
  const resendBlocked = cooldownSeconds > 0 || sendAttempts >= MAX_EMAIL_SEND_ATTEMPTS;
  ...
  return <main className="auth-shell" dir="rtl"><section className="panel auth-card verification-card">
    ...
    <p>{status.status === "SENT" ? "מייל האימות נשלח לשרת הדואר באמצעות הספק הפעיל." : "החשבון ממתין לאימות כתובת הדוא״ל."}</p>
    {status.email && <strong>{maskEmailAddress(status.email)}</strong>}
    <dl className="verification-status"><div><dt>סטטוס השליחה</dt><dd>{statusLabel}</dd></div><div><dt>זמן השליחה האחרון</dt><dd>{status.lastSentAt ? formatIsraelDateTime(status.lastSentAt) : "אין שליחה קודמת"}</dd></div></dl>
