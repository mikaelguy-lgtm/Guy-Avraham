...
import { dealTypeOptions } from "../utils/clientForm";
...
export default function DashboardView({user, clientsOnly = false}: {user: CurrentUser; clientsOnly?: boolean}) {
  ...
  const visibleClients = useMemo(() => clients.filter((client) => {
    ...
    return matchesSearch && (status === "ALL" || client.status === status) && (dealType === "ALL" || client.dealType === dealType);
  }), [clients, search, status, dealType]);
  const stats = [
  ...
  return <main className="advisor-page">
    ...
    <section className="content-card client-list-section">
      ...
      <div className="client-toolbar"><label className="search-field"><Search size={18} aria-hidden="true" /><input aria-label="חיפוש לקוחות" placeholder="חיפוש לפי שם או מספר תיק" value={search} onChange={(event) => setSearch(event.target.value)} /></label><label className="filter-field"><span>סינון לפי סטטוס</span><select aria-label="סינון לפי סטטוס" value={status} onChange={(event) => setStatus(event.target.value)}><option value="ALL">כל הסטטוסים</option><option value="DRAFT">טיוטה</option><option value="ACTIVE">פעיל</option><option value="SUBMITTED">נשלח לחברות מימון</option><option value="CLOSED">נסגר</option></select></label><label className="filter-field"><span>סינון לפי סוג עסקה</span><select aria-label="סינון לפי סוג עסקה" value={dealType} onChange={(event) => setDealType(event.target.value)}><option value="ALL">כל סוגי העסקאות</option>{dealTypeOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label></div>
      {loading ? <div className="empty-state">טוען את תיקי הלקוחות…</div> : visibleClients.length === 0 ? <div className="empty-state"><FolderCheck size={34} /><h3>לא נמצאו לקוחות</h3><p>ניתן לשנות את החיפוש או ליצור תיק לקוח חדש.</p></div> : <div className="clients-grid">{visibleClients.map((client) => <article className="client-card" key={client.id}><div className="client-card-top"><span className="client-avatar">{client.firstName.slice(0, 1)}{client.lastName.slice(0, 1)}</span><span className={`status-badge status-${client.status.toLowerCase()}`}>{formatClientStatus(client.status)}</span></div><div><h3>{client.firstName} {client.lastName}</h3><p className="case-number">תיק {client.publicCaseNumber}</p></div><dl className="client-card-data"><div><dt>סוג עסקה</dt><dd>{formatDealType(client.dealType)}</dd></div><div><dt>סכום מבוקש</dt><dd>{formatCurrency(client.requestedAmount)}</dd></div><div><dt>עודכן לאחרונה</dt><dd>{formatDate(client.updatedAt)}</dd></div></dl><button type="button" className="card-link" onClick={() => navigate(`/advisor/clients/${client.id}`)}>פתיחת התיק<ArrowLeft size={17} /></button></article>)}</div>}
    </section>
  </main>;
}
