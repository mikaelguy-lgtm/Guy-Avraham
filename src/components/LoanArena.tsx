...
export default function LoanArena({clientId, onMissingDocuments, onSent}: {clientId?: number; onMissingDocuments?: () => void; onSent?: () => void}) {
  ...
  return <section className="arena-workspace delivery-flow" aria-busy={busy}>
    ...
    {stage === "complete" && <div className="delivery-complete"><CheckCircle2 /><h2>התיק הוגש בהצלחה{sentCompanyCount ? ` ל-${sentCompanyCount} חברות מימון` : ""}</h2><p className="delivery-followup-note">בתוך 48 שעות תקבל עדכון למייל אם אחת מחברות המימון תביע עניין בתיק.</p></div>}
