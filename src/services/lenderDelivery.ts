...
export class PostgresLenderDeliveryService implements LenderDeliveryApplication {
  ...
  private async loadFullSnapshot(clientId: number, advisorId?: number): Promise<FullCaseSnapshot> {
    const clientResult = await this.pool.query(`
      select c.*, p.property_type, p.property_type_other_description_encrypted, p.city as property_city,
        p.address_encrypted as property_address_encrypted, p.estimated_value, lr.purpose, lr.purpose_other_encrypted, lr.requested_amount,
    ...
    const borrowers = borrowerResult.rows.map((row): FullCaseBorrowerSnapshot => {
      ...
      return {
        order: Number(row.borrower_order), firstName: this.decrypt(row.first_name_encrypted), lastName: this.decrypt(row.last_name_encrypted), identityNumber: this.decrypt(row.identity_number_encrypted), dateOfBirth, age: calculateAge(dateOfBirth), phone: this.decrypt(row.phone_encrypted), email: this.decrypt(row.email_encrypted), address, city, streetAddress, residenceCity: city || residenceCityFromAddress(address), housingStatus: row.housing_status, housingStatusOther: row.housing_status_other_encrypted ? this.decrypt(row.housing_status_other_encrypted) : null, maritalStatus: row.marital_status, numberOfChildren: Number(row.number_of_children), childrenAges: row.children_ages ?? [],
    ...
    return {
      ...
      property: {propertyType: client.property_type, propertyTypeOtherDescription: this.decrypt(client.property_type_other_description_encrypted) || null, city: client.property_city ?? "", address: client.property_address_encrypted ? this.decrypt(client.property_address_encrypted) : null, value: Number(client.estimated_value)},
      loanRequest: {purpose: client.purpose, purposeOther: client.purpose_other_encrypted ? this.decrypt(client.purpose_other_encrypted) : null, requestedAmount: Number(client.requested_amount), requestedTermMonths: Number(client.requested_term_months), loanToValue: Number(client.loan_to_value)},
  ...
  async adminAction(publicId: string, action: string, values: Record<string, unknown>, actor: AdminDeliveryActor, context: DeliveryContext): Promise<unknown> {
    ...
    if (action === "extend-access") {
      ...
      if (!(["PENDING", "PENDING_VERIFICATION"].includes(row.decision_status)) || new Date(row.response_deadline_at).getTime() <= this.now().getTime()) throw new DeliveryError("REMINDER_NOT_AVAILABLE", 409, "לא ניתן לשלוח תזכורת לאחר החלטה או לאחר תום מועד התגובה.");
      if (row.reminder_sent_at) throw new DeliveryError("REMINDER_ALREADY_SENT", 409, "תזכורת כבר נשלחה לחברה זו עבור תיק זה.");
      const recipient = await this.pool.query("select sci.id,lc.email from submission_contact_invitations sci join lender_contacts lc on lc.id=sci.contact_id where sci.company_submission_id=$1 and sci.closed_at is null order by lc.is_primary desc,sci.id limit 1", [row.id]);
      if (!recipient.rows[0]) throw new DeliveryError("REMINDER_NOT_AVAILABLE", 409, "אין איש קשר פעיל לשליחת תזכורת.");
      const claimed = await this.pool.query("update company_submissions set reminder_sent_at=now(),updated_at=now() where id=$1 and reminder_sent_at is null returning id", [row.id]);
      if (claimed.rows[0]) {
        const invitation = recipient.rows[0];
        await this.pool.query(`insert into email_outbox(idempotency_key,template,recipient,payload,status,available_at,company_submission_id,invitation_id) values($1,'LENDER_REMINDER',$2,$3,'PENDING',now(),$4,$5) on conflict(idempotency_key) do nothing`, [`LENDER_REMINDER:${row.id}`, invitation.email, {invitationId: invitation.id, manual: true}, row.id, invitation.id]);
        await this.pool.query("update submission_contact_invitations set reminder_one_sent_at=now(),updated_at=now() where id=$1", [invitation.id]);
      }
    } else if (action === "resend-failed") {
      const invitations = await this.pool.query(`select sci.id,lc.email,(select failed.id from email_outbox failed where failed.invitation_id=sci.id and failed.status='FAILED' order by failed.created_at desc limit 1) failed_outbox_id from submission_contact_invitations sci join lender_contacts lc on lc.id=sci.contact_id where sci.company_submission_id=$1 and sci.status='FAILED' and not exists(select 1 from email_outbox successful where successful.invitation_id=sci.id and successful.status='SENT' and successful.message_id is not null)`, [row.id]);
      if (invitations.rows.length === 0) throw new DeliveryError("RESEND_NOT_AVAILABLE", 409, "אין הודעה שנכשלה וזמינה לשליחה מחדש.");
      for (const invitation of invitations.rows) {
        const idempotencyKey = `admin:resend-failed:${invitation.id}:${invitation.failed_outbox_id ?? "missing"}`;
        const queued = await this.pool.query(`insert into email_outbox(idempotency_key,template,recipient,payload,status,available_at,company_submission_id,invitation_id) values($1,'LENDER_INITIAL',$2,$3,'PENDING',now(),$4,$5) on conflict(idempotency_key) do nothing returning id`, [idempotencyKey, invitation.email, {invitationId: invitation.id, manual: true}, row.id, invitation.id]);
        if (queued.rows[0]) await this.pool.query("update submission_contact_invitations set status='QUEUED',email_failed_at=null,email_failure_reason=null,updated_at=now() where id=$1 and status='FAILED'", [invitation.id]);
      }
  ...
  private async outboxContent(item: Row): Promise<{content: DeliveryEmailContent; invitationId?: number; submissionId?: number}> {
    ...
    if (item.template === "ADVISOR_DECISION") {
      const result = await this.pool.query(`select cs.id,l.name company_name,c.id client_id,u.first_name from company_submissions cs join lenders l on l.id=cs.company_id join case_versions cv on cv.id=cs.case_version_id join clients c on c.id=cv.client_id join advisor_profiles ap on ap.id=cs.advisor_id join users u on u.id=ap.user_id where cs.id=$1`, [payload.submissionId]); const row = result.rows[0]; return {content: deliveryEmailTemplates.advisor({advisorFirstName: row.first_name, companyName: row.company_name, interested: Boolean(payload.interested), url: `${this.appUrl}/advisor/clients/${row.client_id}?tab=company-responses`}), submissionId: Number(row.id)};
    }
  ...
  private async processSchedules(): Promise<void> {
    ...
    for (const row of pending.rows) {
      ...
      const reminderAt = calendar.calculateReminderSchedule(new Date(0), deadline);
      if (now < reminderAt) continue;
      const recipient = await this.pool.query("select sci.id,lc.email from submission_contact_invitations sci join lender_contacts lc on lc.id=sci.contact_id where sci.company_submission_id=$1 and sci.closed_at is null order by lc.is_primary desc,sci.id limit 1", [row.submission_id]);
      if (!recipient.rows[0]) continue;
      const claimed = await this.pool.query("update company_submissions set reminder_sent_at=now(),updated_at=now() where id=$1 and reminder_sent_at is null returning id", [row.submission_id]);
      if (!claimed.rows[0]) continue;
      const invitation = recipient.rows[0];
      await this.pool.query(`insert into email_outbox(idempotency_key,template,recipient,payload,status,available_at,company_submission_id,invitation_id) values($1,'LENDER_REMINDER',$2,$3,'PENDING',now(),$4,$5) on conflict(idempotency_key) do nothing`, [`LENDER_REMINDER:${row.submission_id}`, invitation.email, {invitationId: invitation.id}, row.submission_id, invitation.id]);
      await this.pool.query("update submission_contact_invitations set reminder_one_sent_at=now(),updated_at=now() where id=$1", [invitation.id]);
    }
