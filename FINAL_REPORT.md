...
## 14. החלפת מודל ההתחייבויות ואכיפת מסמכי חובה

### Migration ומבנה נתונים

- נוצרה Migration בטוחה ולא הרסנית: `drizzle/0006_married_molten_man.sql`.
- טבלת `liabilities` כוללת כעת שיוך לתיק וללווה, `scope` מסוג `BORROWER`/`HOUSEHOLD`, סוג התחייבות, יתרה נוכחית, החזר חודשי, תאריך סיום, תיאור גוף אחר מוצפן, הערות מוצפנות, סטטוס Legacy ו־Soft Delete.
- התחייבות ברמת `BORROWER` מחייבת `borrower_id`; התחייבות ברמת `HOUSEHOLD` מחייבת `borrower_id = NULL`. קיימים Foreign Keys, אינדקסים ו־Database constraints.
- נתוני המודל הישן הועברו ללא המצאת יתרה או תאריך סיום וסומנו `INCOMPLETE_LEGACY`; תיק כזה חסום לשליחה עד השלמת הנתונים.
- נוספו ללקוחות פרטי עסקה מוצפנים ומטא־דאטה של עדכון, ולמסמכים נוספו `borrower_id`, ‏`custom_title`, תיאור מוצפן וסטטוס `REPLACED`.
- נוספה מטרת הלוואה `BRIDGE_FINANCING`, המוצגת בעברית כ־"גישור".

### כללי התחייבויות

- ניתן להוסיף ולמחוק מספר בלתי מוגבל של התחייבויות דינמיות, עם סוג, יתרה, החזר, תאריך סיום, תקופה מחושבת והערות.
- בתיק נשוי נשמרת רשימת `HOUSEHOLD` אחת בלבד; בתיקים עם קשר אחר נשמרות רשימות `BORROWER` נפרדות לכל לווה.
- הסיכומים מחושבים מהשורות הפעילות בלבד: מספר התחייבויות, סך יתרות וסך החזרים חודשיים.
- `calculateRemainingCommitmentPeriod` מחשב בזמן אמת שנים, חודשים וימים ואינו שומר נתון מתיישן במסד.
- השדות הישנים של התחייבויות חודשיות, יתרת משכנתה והחזר משכנתה אינם בשימוש ב־UI או ב־DTO החדש.

### מסמכים וחסימת שליחה

- לכל לווה נדרשים: תעודת זהות צד קדמי, תעודת זהות צד אחורי וספח תעודת זהות.
- לכל תיק נדרשים: נסח טאבו או אישור זכויות וכתב הסמכה.
- מסמך "אחר" תומך במספר העלאות ובשם מותאם; השם אינו נכלל ב־Snapshot האנונימי.
- `GET /api/clients/:clientId/documents/requirements` מחזיר רשימת חוסרים מסודרת.
- `POST /api/clients/:clientId/submissions` מחזיר `422` עם קוד בטוח ורשימת המסמכים החסרים, או עם `INCOMPLETE_LEGACY_LIABILITIES`, לפני יצירת PDF, הזמנה, רשומת Submission או שליחת דוא״ל.
- לאחר השלמת המסמכים נוצר Snapshot מסוכם בלבד; הערות התחייבות, שמות גופים, פירוט עסקה, כתובת מלאה ו־PII אינם נכללים בו.

### UI ו־E2E

- אשף הלקוח ומסך העריכה משתמשים במודל הדינמי, מציגים עברית מלאה ו־RTL, ואינם מציגים אזור, תקופת הלוואה, יחס החזר או אחוז מימון.
- מסך הלקוח כולל את סדר הכרטיסיות החדש ואת הטאב "פירוט עסקה"; טאב חברות המימון הוסר, בעוד זירת המימון וכפתור השליחה נשארו פעילים.
- מסך המסמכים מציג Checklist לפי תיק ולווה, Progress, העלאה, צפייה, הורדה, החלפה ומחיקה.
- Playwright אימת 15 מטרות הלוואה, תיק נשוי עם התחייבויות משותפות, תיק שותפים עם נתונים נפרדים, חסימת מסמכים, העלאה ל־MinIO, שליחה ל־Mailpit, פורטל חתם, חשיפה חלקית והצעה.
...
npm run typecheck         PASS
npm run lint              PASS
npm run test:unit         PASS — 60/60
npm run test:integration  PASS — 67/67
