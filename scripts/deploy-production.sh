...
require_environment_names \
  APP_URL PUBLIC_APP_URL API_BASE_URL EXTERNAL_REVIEW_BASE_URL EXTERNAL_ACCESS_BASE_URL ALLOWED_ORIGINS \
  ...
  SECRET_PROVIDER GOOGLE_CLOUD_PROJECT GOOGLE_APPLICATION_CREDENTIALS_HOST_PATH FIREBASE_PROJECT_ID FIREBASE_CLIENT_EMAIL \
...
if [[ ! -f "$GOOGLE_APPLICATION_CREDENTIALS_HOST_PATH" || "$(stat -c '%a' "$GOOGLE_APPLICATION_CREDENTIALS_HOST_PATH")" != "600" ]]; then
