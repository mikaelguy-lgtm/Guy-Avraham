...
export function makeStore(overrides: Partial<AppStore> = {}): AppStore {
  ...
  const defaults: Partial<AppStore> = {
    ...
    getLenderSubmission: async (id) => id === 1 ? {id: 1, lenderId: 100, clientId: 1, status: "SENT", anonymousSnapshot: {publicCaseNumber: "SC-1", dealType: "SECOND_HAND_PURCHASE", propertyType: "APARTMENT", propertyRegion: "CENTER", propertyValue: 2_000_000, requestedAmount: 1_000_000, financingPercentage: 50, numberOfBorrowers: 2, borrowerRelationship: "COUPLE", borrowerAges: [41, 39], employmentTypes: ["SALARIED", "SELF_EMPLOYED"], totalMonthlyIncome: 50_000, totalMonthlyPayments: 6_000, existingMortgageBalance: 400_000, requestedTermMonths: 240}} : null,
