...
describe("focused client validation", () => {
  it("validates each section independently", () => {
    ...
    expect(clientPropertyInputSchema.parse({loanPurpose: "SECOND_HAND_PURCHASE", property: {propertyType: "APARTMENT", propertyTypeOtherDescription: null, city: "תל אביב", address: "רחוב", value: 0}, loanRequest: {requestedAmount: 0}}).property.value).toBe(0);
  });
