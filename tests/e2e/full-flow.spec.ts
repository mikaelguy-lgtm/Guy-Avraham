...
const clientPayload = {
  ...
  loanPurpose: "SECOND_HAND_PURCHASE", loanPurposeOther: null, loanRequest: {requestedAmount: 1_000_000}, dealDetails: "בדיקת מסירה מבקשת מימון. delivery-client@syncash.local ומעסיק סודי בע״מ", status: "ACTIVE"
};
...
test("advisor-to-company delivery uses one OTP and a persistent seven-day portal grant", async ({browser, page, request}) => {
  ...
  try {
    ...
    expect(preview.pdfRendererVersion).toBe(5); expect(preview.pdfFontFingerprint).toMatch(/^[a-f0-9]{64}$/); expect(preview.pdfContentHash).toMatch(/^[a-f0-9]{64}$/);
    ...
    expect(regeneratedObject.Metadata?.["renderer-version"]).toBe("5"); expect(regeneratedObject.Metadata?.["font-fingerprint"]).toMatch(/^[a-f0-9]{64}$/); expect(regeneratedObject.Metadata?.["content-hash"]).toMatch(/^[a-f0-9]{64}$/);
