...
test("advisor visual journey is Hebrew, RTL and responsive", async ({page, request}) => {
  ...
  try {
    ...
    await expect(page.getByLabel("מצב משפחתי").locator('option[value="SEPARATED"]')).toHaveCount(0);
    ...
    await expect(page.getByLabel("סוג תעסוקה").locator('option[value="GOVERNMENT_EMPLOYEE"]')).toHaveCount(0);
    await expect(page.getByLabel("סוג תעסוקה").locator('option[value="SECURITY_FORCES"]')).toHaveCount(0);
    await expect(page.getByLabel("סוג תעסוקה").locator('option[value="TORAH_INSTITUTION"]')).toHaveText("מוסד תורני");
    ...
    await page.getByLabel("סכום הכנסה חודשי").fill("2000");
    await page.getByRole("button", {name: "הוספת הכנסה נוספת"}).click();
    await page.getByLabel("סוג הכנסה נוספת").nth(1).selectOption("SALARIED");
    await page.getByLabel("סכום הכנסה חודשי").nth(1).fill("0");
    await expect(page.getByRole("heading", {name: /הכנסה נוספת/})).toHaveCount(2);
    await page.getByRole("button", {name: "מחיקה"}).nth(1).click();
    await expect(page.getByRole("heading", {name: /הכנסה נוספת/})).toHaveCount(1);
    await page.getByRole("button", {name: "הוספת התחייבות"}).click();
    await page.getByLabel("סוג התחייבות 1").selectOption("MORTGAGE");
    await expect(page.getByLabel("יתרה נוכחית")).toBeVisible();
    await expect(page.getByLabel("הגוף הפיננסי")).toBeVisible();
    await page.getByLabel("סוג התחייבות 1").selectOption("RENT");
    await expect(page.getByLabel("יתרה נוכחית")).toHaveCount(0);
    await expect(page.getByLabel("הגוף הפיננסי")).toHaveCount(0);
    await page.getByLabel("סוג התחייבות 1").selectOption("MORTGAGE");
