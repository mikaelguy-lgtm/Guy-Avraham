...
| "יתרה נוכחית" not shown/required, stays null (never 0), for ALIMONY/RENT | **Done** | Column has no default, never coerced to 0 anywhere in `store.ts`; hidden in the UI for ALIMONY/RENT; unaffected for LOAN/MORTGAGE (existing business rule keeps applying there). DB-level `liabilities_balance_relevance_check` only names `RENT` explicitly (app-level Zod check additionally covers `ALIMONY`) — a minor defense-in-depth gap, not a bug (see `docs/TODO.md`). |
| Resetting `financialInstitution`/`currentBalance` to null when the liability type is changed away from a type that used them | **Done at the form-state and payload layers** | `ClientFormFields.tsx:47` (`updateType`): switching to `ALIMONY`/`RENT` clears `currentBalance`; switching away from `LOAN`/`MORTGAGE` clears `financialInstitution`. `clientForm.ts:201` (`liabilityPayload`): independently forces both fields to `null` based on the *final* selected type before the API payload is built, regardless of what stale value sits in form state — so no hidden stale value can reach the API from the standard UI flow. Not yet specifically verified against a raw/partial API `PATCH` bypassing the UI (see `docs/TODO.md`). |
